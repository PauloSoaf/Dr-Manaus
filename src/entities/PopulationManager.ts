...
/** Props keep the ids they had when vehicles shared this pool, so saved destruction still resolves. */
const FIRST_PROP = 28, LAST_PROP = 76;
...
export class PopulationManager {
  ...
  npcCount = 24; reaction = 0;
  /**
   * Cars are no longer simulated here: they live on the real road graph in TrafficSystem, which is
   * the only place a car can legally be. The field stays because the quality presets set it, and
   * Game forwards it to `TrafficSystem.setCount`.
   */
  vehicleCount = 16;
  ...
  update(dt:number,player:Vector3,size:number) {
    ...
    for(const body of this.targets) {
      ...
      if(enabled) {
        ...
        this.colliders.push({x:body.position.x,y:body.position.y,z:body.position.z,width:1.2,height:1.4,depth:1.2,id:body.id});
      }
      this.dummy.position.copy(body.position);this.dummy.rotation.set(0,body.angle,0);this.dummy.scale.setScalar(enabled?1.2:0);this.dummy.updateMatrix();
      this.props.setMatrixAt(body.index,this.dummy.matrix);
    }
    for(let i=0;i<40;i++) {
