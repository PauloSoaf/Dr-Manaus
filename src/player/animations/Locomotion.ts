...
export class Locomotion {
  ...
  apply(bones: readonly Bone[], dt: number, speed: number, flying: boolean, pose: string): void {
    const name = pose === 'punch' || pose === 'punchCross' ? pose : flying ? '' : pose === 'jump' || pose === 'vault' || pose === 'roll' ? pose : !pose && speed > .4 ? speed > 8 ? 'run' : 'walk' : '';
    ...
    const time = name.startsWith('punch') ? Math.min(this.time / .42, 1) * clip.duration : name === 'vault' || name === 'roll' ? Math.min(this.time, clip.duration) : this.time % clip.duration;
