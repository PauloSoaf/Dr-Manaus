...
export class Locomotion {
  ...
  apply(bones: readonly Bone[], dt: number, speed: number, flying: boolean, pose: string): void {
    const name = flying ? '' : pose === 'jump' || pose === 'vault' || pose === 'punch' ? pose : !pose && speed > .4 ? speed > 8 ? 'run' : 'walk' : '';
    ...
    const time = name === 'punch' || name === 'vault' ? Math.min(this.time, clip.duration) : this.time % clip.duration;
